		...
		<div class="row">
			...
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="x_panel">
					<!-- X-title -->
					<div class="x_title">
						<h2>DANH SÁCH TỆP ĐÍNH KÈM</h2>
						<a href="{{ route('admin.cuocthifile.create') }}" class="btn btn-primary pull-right">Thêm mới</a>
						<div class="clearfix"></div>
					</div>
					<!-- X-title -->
					...
					<div class="x_content">
						<table class="table" id="table-category">
							...
							<tbody>
								...
								<tr>
									<td>{{ $key+1 }}</td>
									<td>{{ $value->cuocthi->ten }}</td>
									<td>{{ $value->ten_tep }}</td>
									<td><a href="{{ $value->url }}"><b>Tải xuống</b></a></td>
									<td>
										<a href="{{ route('admin.cuocthifile.edit', ['id'=>$value->id]) }}" class="btn btn-warning btn-xs">Sửa</a>
										<a href="{{ route('admin.cuocthifile.delete', ['id'=>$value->id]) }}" class="btn btn-danger btn-xs">Xóa</a>
									</td>
								</tr>
								@endforeach
							</tbody>
